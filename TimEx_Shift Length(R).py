...
for l in L:
    for n in N:
                    ...
                    while ((l-len(val))>=0):
                        ...
                        if (len(val) > 0):
                            ...
                            if (len(UAE_ACTUAL[val[0]:val[-1]+1])==l and DEV_SUB >= Dev_ref):
                                ...
                                if (oper_list[i-1]-val[0]) >= 0:
                                    combine_prune_paned_actual += (((oper_list[i-1]-val[0]+1)/g)-1)
                                else:
                                    combine_prune_paned_actual += (2*((oper_list[i-1]-val[0]+1)/g)-1)
        ...
        Pane_Prune_dict[l] = Pane_Prune_list
        Pane_Prune_savings_dict[l] = Pane_Prune_savings_list
        Pane_no_prune_dict[l] = Pane_no_prune_list
        Pane_per_prune_dict[l] = Pane_per_prune_list
        Pane_per_prune_savings_dict[l] = Pane_per_prune_savings_list
        Pane_L_dict[l] = Pane_L_list
        Pane_S_dict[l] = Pane_S_list
        
        Pane_share_dict[l] = Pane_share_list
        Pane_share_savings_dict[l] = Pane_share_savings_list
        Pane_per_share_dict[l] = Pane_per_share_list
        Pane_per_share_savings_dict[l] = Pane_per_share_savings_list
        
        share_pane_dict[l] = share_pane_list
        share_pane_savings_dict[l] = share_pane_savings_list
        per_share_pane_dict[l] = per_share_pane_list
        per_share_pane_savings_dict[l] = per_share_pane_savings_list
        
        
        combine_prune_paned_savings_dict[l] = combine_prune_paned_savings_list
        per_combine_prune_paned_savings_dict[l] = per_combine_prune_paned_savings_list
        combine_prune_paned_actual_dict[l] = combine_prune_paned_actual_list
        per_combine_prune_paned_actual_dict[l] = per_combine_prune_paned_actual_list 

        no_prune_dict[l] = no_prune_list
        no_prune_savings_dict[l] = no_prune_savings_list
        per_no_prune_dict[l] = per_no_prune_list
        per_no_prune_savings_dict[l] = per_no_prune_savings_list
...
for l in L:    
    for n in N:
        ...
        Prune_point_actual_dict[l] = Prune_point_actual_list
        Prune_point_dict[l] = Prune_point_list
        no_prune_point_dict[l] = no_prune_point_list
        per_prune_point_dict[l] = per_prune_point_list
        per_prune_point_actual_dict[l] = per_prune_point_actual_list
        L_point_dict[l] = L_point_list
        L_dict[l] = L_list
        S_dict[l] = S_list
...
for p in L:
    plt.plot(S_dict[p], no_prune_savings_dict[p], label='Linear Search', marker = 'o', color = 'b')
    plt.plot(S_dict[p], Prune_point_dict[p], label='TimEx-pruning', marker = 'x', color = 'orange')
    #plt.plot(S_dict[p], Pane_Prune_savings_dict[p], label='Pane Horizontal pruning', marker = 'o')
    plt.plot(S_dict[p], share_pane_savings_dict[p], label='TimEx-sharing', marker = '.', color = 'g')
    plt.plot(S_dict[p], combine_prune_paned_savings_dict[p], label='TimEx-hybrid', marker = 'D', color = 'r')
    plt.xticks(ticks=S_dict[p], labels=S_dict[p], rotation=45)


plt.xlabel("Shift Length (S)")
...
for p in L:
    plt.plot(S_dict[p], no_prune_dict[p], label='Linear Search', marker = 'o', color = 'b')
    plt.plot(S_dict[p], Prune_point_actual_dict[p], label='TimEx-pruning', marker = 'x', color = 'orange')
    #plt.plot(S_dict[p], Pane_Prune_dict[p], label='Pane Horizontal pruning', marker = 'o')
    plt.plot(S_dict[p], share_pane_dict[p], label='TimEx-sharing', marker = '.', color = 'g')
    plt.plot(S_dict[p], combine_prune_paned_actual_dict[p], label='TimEx-hybrid', marker = 'D', color = 'r')
    plt.xticks(ticks=S_dict[p], labels=S_dict[p], rotation=45)
...
for p in L:
    #plt.plot(S_dict[p], per_no_prune_savings_dict[p], label='Linear Search', marker = 'o', color = 'b')
    plt.plot(S_dict[p], per_prune_point_dict[p], label='TimEx-pruning', marker = 'x', color = 'orange')
    #plt.plot(S_dict[p], Pane_per_prune_savings_dict[p], label='Pane Horizontal pruning', marker = 'o')
    plt.plot(S_dict[p], per_share_pane_savings_dict[p], label='TimEx-sharing', marker = '.', color = 'g')
    plt.plot(S_dict[p], per_combine_prune_paned_savings_dict[p], label='TimEx-hybrid', marker = 'D', color = 'r')
    plt.xticks(ticks=S_dict[p], labels=S_dict[p], rotation=45)
...
for p in L:
    #plt.plot(S_dict[p], per_no_prune_dict[p], label='Linear Search', marker = 'o', color = 'b')
    plt.plot(S_dict[p], per_prune_point_actual_dict[p], label='TimEx-pruning', marker = 'x', color = 'orange')
    #plt.plot(S_dict[p], Pane_per_prune_dict[p], label='Pane Horizontal pruning', marker = 'o')
    plt.plot(S_dict[p], per_share_pane_dict[p], label='TimEx-sharing', marker = '.', color = 'g')
    plt.plot(S_dict[p], per_combine_prune_paned_actual_dict[p], label='TimEx-hybrid', marker = 'D', color = 'r')
    plt.xticks(ticks=S_dict[p], labels=S_dict[p], rotation=45)
